...
![Text slide titled "Metabolic dynamics and ventilation during exercise" listing the overview (fundamentals of gas exchange, oxygen supply cascade; changes in VO2 and energy supply during exercise; ventilatory responses to exercise) and four learning objectives.](images/lec05/slide-002.png)

### Overview and Learning Objectives

**Overview:**
- Fundamentals of gas exchange and the oxygen supply cascade
- Changes in VO<sub>2</sub> and energy supply during exercise
...
![Diagram showing the steps in the oxygen supply cascade. Step 1: Pulmonary ventilation from environment to alveoli, governed by gas laws and the Fick principle, with the equation for VO2 based on alveolar variables. Step 2: Alveolar gas exchange between the lungs and capillary blood, governed by Fick's law of diffusion and diffusion capacity, with equation VO2 = DL(PA,O2 - Pa,O2). Arrows indicate that partial pressure gradients drive gas transport.](images/lec05/slide-003.png)
...
![Slide showing factors that influence alveolar PO2 and PCO2. A photo of a bell jar lung model is shown alongside a graph of PO2 versus altitude, and images of indoor CO2 monitors. The PO2 of inspired air is influenced by altitude; the PCO2 is influenced by air quality and indoor ventilation systems.](images/lec05/slide-004.png)
...
![Slide showing factors that influence alveolar PO2 and PCO2, focusing on minute/alveolar ventilation. Left: a bell jar lung model diagram with PAO2 labeled. Right: a graph plotting partial pressure (mmHg) on the y-axis versus alveolar ventilation (L/min) on the x-axis, assuming constant metabolic rate. The PO2 curve (green) rises with increasing ventilation, while the PCO2 curve (magenta) decreases. Arrows indicate hypoventilation on the left and hyperventilation on the right.](images/lec05/slide-005.png)
...
![Slide showing factors that influence alveolar PO2 and PCO2, focusing on metabolic rate of O2 consumption and CO2 production. Left: a bell jar lung model diagram. Right: a graph plotting partial pressure (mmHg) on the y-axis versus metabolic rate (mL/min) on the x-axis, assuming constant alveolar ventilation. The PCO2 curve (magenta) rises with increasing metabolic rate, while the PO2 curve (green) decreases.](images/lec05/slide-006.png)
...
![Slide titled "Calculating PO2 and PCO2" showing two equations. The clinical alveolar ventilation equation for calculating PACO2: PACO2 = (VCO2 / VA) times K, where K = 863 mmHg (a constant depending on units when flow rates are in L/min BTPS and VCO2 is in mL/min STPD). Below, the alveolar gas equation for calculating PAO2: PAO2 = PIO2 minus (PACO2 / R) times a correction factor. Together these equations allow calculation of alveolar PO2 and PCO2 from non-invasive external measures.](images/lec05/slide-007.png)

### Calculating Alveolar P<sub>O₂</sub> and P<sub>CO₂</sub>
...
![Section title slide with a lavender background reading "Changes in metabolic rate in response to exercise."](images/lec05/slide-008.png)

- This section transitions to the core topic of the lecture: how metabolic rate (measured as VO<sub>2</sub>) changes in response to exercise, including the equipment used, calculation methods, and the relationship between exercise intensity and oxygen consumption.
...
### Measurement of VO<sub>2</sub>

- **VO<sub>2</sub>** (volume rate of oxygen consumption) is typically measured in mL O<sub>2</sub>/min and is a fundamental measure of metabolic rate.
- Three essential pieces of equipment are required:
  1. **Gas analyzers** — measure fractional concentrations of O<sub>2</sub> and CO<sub>2</sub> in exhaled air. CO<sub>2</sub> analyzers are generally cheaper and more readily available than O<sub>2</sub> analyzers. If only one gas is measured, a specific **respiratory exchange ratio (R)** must be assumed.
...
### Calculation of Mass-Specific VO<sub>2</sub>

- The equation for VO<sub>2</sub>:
...
![Graph titled "Walking and Running speed and steady state VO2." The x-axis shows walking/running speed in m/min (0 to 250), and the y-axis shows VO2 in mL/kg/min (0 to 60). Two red regression lines are plotted: a lower line for walking (VO2 = 0.1x + 3.5 mL/kg/min) and a steeper upper line for running (VO2 = 0.2x + 3.5 mL/kg/min). Running is more metabolically expensive than walking at any given speed, and has a steeper slope.](images/lec05/slide-010.png)
...
![Graph titled "Cycling work rate and VO2." The x-axis shows cycle ergometer work rate in watts (50 to 250), and the y-axis shows VO2 in mL/kg/min (0 to 40). A teal line with data points shows an approximately linear increase in VO2 from about 15 mL/kg/min at 50 watts to about 37 mL/kg/min at 200 watts.](images/lec05/slide-011.png)
...
![Bar chart titled "VO2 in running in elite, good, and untrained runners." The y-axis shows VO2 in mL/kg/km (150 to 210). Three orange bars are shown: elite runners (~180), good runners (~192), and untrained runners (~200). A bracket indicates a 10% difference between elite and untrained runners. Elite runners have the lowest metabolic cost per kilometer.](images/lec05/slide-012.png)
...
![Slide titled "Metabolic dynamics: transition from rest to exercise" showing three ATP production pathways. Immediate energy: H2O + ATP produces H+ + ADP + Pi, and H+ + ADP + phosphocreatine (PCr) produces ATP + Creatine. Anaerobic glycolysis: Glucose (C6H12O6) + 2 NAD+ + 2 ADP + 2Pi produces 2 pyruvate (3C) + 2 NADH + 2 H+ + 2 ATP. Aerobic metabolism: Glucose (C6H12O6) + 6O2 produces 6CO2 + 6H2O + 36 ATP (theoretical yield; actual yield is estimated to be 29-32 ATP).](images/lec05/slide-015.png)
...
![Text slide titled "Rapid and slow portions of EPOC." The rapid portion of EPOC includes: resynthesis of stored PCr in muscle, replenishing oxygen in muscle myoglobin, and replenishing blood hemoglobin O2 stores. The slow portion of EPOC includes: increased body temperature (thermoregulatory cost), conversion of lactic acid to glucose (gluconeogenesis), and increased blood levels of epinephrine and norepinephrine. A small graph of EPOC is shown with both components labeled.](images/lec05/slide-018.png)
...
![Graph titled "Oxygen consumption (VO2) during an incremental exercise test." The x-axis shows work rate in watts (50 to 300), and the y-axis shows VO2 in L/min (approximately 0.8 to 4.0). Data points connected by a green line show a linear increase in steady-state VO2 up to approximately 200 watts, where the curve begins to flatten. A dashed line above the plateau is labeled "VO2 max." Text notes that each point is the steady state value measured once a plateau is reached.](images/lec05/slide-019.png)
...
![Slide titled "Exercise induced hypoxemia in elite athletes (EIAH)." Left: Graphs showing the same experiment performed on thoroughbred racehorses. Oxygen uptake and arterial saturation are plotted against treadmill speed under normoxia and supplemental oxygen conditions. Horses also show exercise-induced hypoxemia at high treadmill speeds under normoxia, which is mostly corrected by supplemental oxygen. Right: A photograph of thoroughbred racehorses racing.](images/lec05/slide-026.png)
...
![Scatter plot titled "Exercise induced hypoxemia in elite athletes (EIAH)" showing arterial PaO2 (mmHg) on the y-axis versus VO2max (mL/kg/min) on the x-axis for a large sample of athletes, measured at 90-100% VO2max. A regression line with R = -0.64 shows a negative correlation: athletes with higher VO2max have lower PaO2 at maximal exercise. Reference: Hopkins, 2006.](images/lec05/slide-027.png)
...
|----------|------|-------------|
| $\dot{V}O\_2 = \dot{V}\_E \times (F\_IO\_2 - F\_EO\_2)$ | VO<sub>2</sub> from expired air | Oxygen consumption rate from minute ventilation and the difference in inspired vs. expired O<sub>2</sub> fractions |
| $\text{Mass-specific } \dot{V}O\_2 = \frac{\dot{V}O\_2}{m\_{body}}$ | Mass-specific VO<sub>2</sub> | VO<sub>2</sub> divided by body mass; standard units are mL/kg/min |
| $\dot{V}O\_2 = 0.1x + 3.5$ | Walking VO<sub>2</sub> | Estimated VO<sub>2</sub> (mL/kg/min) for walking at speed $x$ (m/min) |
| $\dot{V}O\_2 = 0.2x + 3.5$ | Running VO<sub>2</sub> | Estimated VO<sub>2</sub> (mL/kg/min) for running at speed $x$ (m/min) |
| $P\_ACO\_2 = \frac{\dot{V}CO\_2}{\dot{V}\_A} \times K$ | Clinical alveolar ventilation equation | Calculates alveolar P<sub>CO₂</sub> from CO<sub>2</sub> production rate and alveolar ventilation; $K = 863$ mmHg |
| $P\_AO\_2 = P\_IO\_2 - \frac{P\_ACO\_2}{R}$ | Alveolar gas equation (simplified) | Estimates alveolar P<sub>O₂</sub> from inspired P<sub>O₂</sub>, alveolar P<sub>CO₂</sub>, and respiratory exchange ratio $R$ |
...
| Term | Definition |
|------|-----------|
| **VO<sub>2</sub>** | Volume rate of oxygen consumption; the standard measure of metabolic rate, typically expressed in mL O<sub>2</sub>/min or mL/kg/min. |
| **Mass-specific VO<sub>2</sub>** | Oxygen consumption rate normalized to body mass (mL/kg/min), used for comparing metabolic rates across individuals or species. |
| **VO<sub>2</sub>max** | The maximum rate of oxygen consumption attainable during incremental exercise; represents the upper limit of aerobic metabolism. |
...
| **Respiratory exchange ratio (R)** | The ratio of CO<sub>2</sub> produced to O<sub>2</sub> consumed (VCO<sub>2</sub>/VO<sub>2</sub>); reflects the metabolic substrate being used. |
| **Running economy** | The metabolic cost of running at a given speed, expressed as O<sub>2</sub> consumption per unit distance (mL/kg/km); lower values indicate greater efficiency. |
| **Oxygen deficit** | The difference between the total energy demand at the onset of exercise and the energy supplied aerobically during the period before VO<sub>2</sub> reaches steady state. |
...
| **Aerobic metabolism (Oxidative phosphorylation)** | The metabolic pathway that fully oxidizes glucose using O<sub>2</sub> in the mitochondria, theoretically yielding 36 ATP (actual yield ~29-32 ATP). |
| **Ventilatory threshold** | The exercise intensity (typically 50-70% VO<sub>2</sub>max) at which ventilation begins to increase exponentially rather than linearly with work rate. |
...
| **Myoglobin** | An oxygen-binding protein in muscle tissue that serves as an oxygen store and buffer, facilitating O<sub>2</sub> diffusion to mitochondria. |
...
| **Incremental exercise test** | A standardized protocol for measuring VO<sub>2</sub>max in which work rate is progressively increased at regular intervals until the subject reaches exhaustion or VO<sub>2</sub> plateaus. |
| **Hyperventilation** | Breathing in excess of metabolic demand, leading to reduced P<sub>A</sub>CO<sub>2</sub> and elevated P<sub>A</sub>O<sub>2</sub>. |
| **Hypoventilation** | Breathing below metabolic demand, leading to elevated P<sub>A</sub>CO<sub>2</sub> and reduced P<sub>A</sub>O<sub>2</sub>. |
