---
layout: lecture
title: "Lecture 17: Forces and Energy Cost in Locomotion (Part 2)"
week: 8
lecture: 17
week_title: "Week 8: Energy Cost of Locomotion and Humans in Comparative Context"
week_url: /week08/
total_slides: 35
prev_lecture: /week07/friday-review
...
![Slide titled "How do forces vary with body size?" with subhead "10⁶ – fold range in body mass among terrestrial mammals." Photograph shows the dramatic contrast between an elephant's foot (filling most of the frame) and a small mouse standing on the same ground beside it.](images/lec17/slide-004.png)
...
![Slide titled "Body size and shape: Scaling of limb posture" with a photograph at top-left showing an elephant's foot dwarfing a small mouse standing nearby. To the right, a schematic comparison: a small upright bipedal animal silhouette is shown with a long limb labeled L³ (volume/mass), and beside it a smaller box labeled L² (cross-sectional area). Annotated text reads: "weight scales faster than musculoskeletal strength."](images/lec17/slide-006.png)
...
![Slide titled "A bit tricky with an elephant! But, it has been done…" Photograph shows an elephant fitted with a custom-built respirometry mask made from a large repurposed plastic garbage bin attached over the trunk; a pump and analyzer rig sits on a small golf cart beside the elephant; two researchers walk alongside, leading the elephant.](images/lec17/slide-014.png)
...
![Slide titled "Energetic cost of transport (CoT) of swimming, flying, running" with a large log-log scatter plot reproduced from Tucker 1975 and Schmidt-Nielsen 1972. X-axis: body mass (kg, log scale from 10⁻⁶ to 10⁷). Y-axis: minimum cost of transport P_t/(W·V) (dimensionless, log scale from 0.1 to 10+). Three diagonal trend lines run downward-right: dashed blue "Fliers" (insect, bird, mammal fliers — mosquito, fly, bee, blowfly, horsefly, locust, hummingbird, budgerigar, crow, bat, pigeon, goose) at the top; solid green "Runners" (mammal, bird, reptile runners — mice, lizards, rat, quail, rabbit, snake, dog, cheetah, kangaroo, horse, human) in the middle; dashed magenta "Swimmers" (fish, mammal swimmers) at the bottom. A horizontal dotted line labeled "migratory line P_t/(WV) = 2" cuts across the middle. A human data point is highlighted with an arrow labeled "humans: higher CoT than expected for our body size." Grey square data points along the bottom right represent man-made machines: ice skater, bicyclist, pedal airplane, hovercraft, Cadillac, DC-8 jet, VW, light plane, helicopter, F-105 jet, truck, freight steamer, freight train, dirigible. A left-side panel summarizes: "Cost of transport (CoT) — energy used per unit distance, per unit body mass. CoT = work/(mass·distance) = power/(mass·velocity). Lower CoT in larger animals."](images/lec17/slide-016.png)
...
### CoT Predicted by 1/L<sub>c</sub>
...
![Transition slide with title "How does cost of transport (CoT) change with speed and gait?" centered on a blank background.](images/lec17/slide-021.png)
...
![Slide repeating the Hoyt & Taylor 1981 CoT-vs-speed figure with the three U-shaped curves for walk, trot, and gallop, with voluntary-speed histograms below. Two bullet annotations on the left: "Gait transitions occur near intersections of the CoT curves"; "Horses prefer speeds within each gait that minimize CoT." Citation: Hoyt & Taylor (1981).](images/lec17/slide-024.png)
...
![Slide titled "What are some limitations of classic studies on locomotion?" with two paragraphs in dark blue. First paragraph: "Focus mainly on steady-state locomotion in the lab — How well does this relate to 'real world' conditions?" Second paragraph: "Limited sample size and mostly ignores sources of individual variation — What factors influence variation in gait and activity across individuals?"](images/lec17/slide-029.png)
...
![Slide titled "Force and energy demands in locomotion" with the same six numbered learning objectives as Slide 12, reproduced verbatim.](images/lec17/slide-034.png)
...
- Closing repeat of the learning objectives. Each is now backed by data from the lecture:
  ...
  5. Metabolic rate predicted by 1/T<sub>c</sub>; CoT predicted by 1/L<sub>c</sub>.
...
|----------|------|-------------|
| $\dot{V}O_2 = \dot{V}_E (F_I O_2 - F_E O_2)$ | Fick principle | Oxygen consumption equals ventilation rate times the difference between inspired and expired O<sub>2</sub> fractions. The basis of respirometry. |
| $\text{CoT} = \dot{V}O_2 / \text{speed}$ | Mass-specific cost of transport | Energy used per unit distance traveled, per unit body mass — typically expressed as mL O<sub>2</sub> kg<sup>−1</sup> m<sup>−1</sup> or J kg<sup>−1</sup> m<sup>−1</sup>. |
...
| $E\_{\text{cot}}/W\_b = C \cdot (1/L\_c)$ | Kram & Taylor cost-of-transport equation | CoT per unit body weight equals the cost coefficient $C$ divided by step length $L\_c$. Larger animals have longer L<sub>c</sub>, so lower CoT. |
...
| Term | Definition |
|------|-----------|
| **Cost of transport (CoT)** | Energy used per unit distance traveled, per unit body mass. Standard metric for comparing locomotor economy across species and speeds. SI units: J kg<sup>−1</sup> m<sup>−1</sup>. |
...
| **Stance (contact) time, T<sub>c</sub>** | Duration that one foot is in contact with the ground during a single step. Small animals have short T<sub>c</sub> and must therefore activate muscles at higher frequencies. |
| **Step length, L<sub>c</sub>** | Distance the body moves forward during one stance. Increases with body size; longer L<sub>c</sub> → lower CoT in the Kram & Taylor framework. |
...
| **Oxycaloric coefficient** | Energy released per unit oxygen consumed during aerobic metabolism. ~20.1 J/mL O<sub>2</sub> on average (~21 J/mL O<sub>2</sub> for carbohydrate, ~19.6 J/mL O<sub>2</sub> for lipid). Used to convert respirometry data to joules. |
...
| **Inverted-pendulum walking** | Walking gait in which a relatively stiff leg vaults the body up and over with E<sub>g</sub> and E<sub>k</sub> exchanged out of phase, minimizing muscular work within stance. |
| **Bouncing (elastic) running** | Running gait in which a compliant leg cycles mechanical energy through elastic tendons and ligaments, with E<sub>g</sub> and E<sub>k</sub> in phase. |
